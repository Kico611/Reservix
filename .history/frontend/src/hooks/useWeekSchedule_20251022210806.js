...
export function useWeekSchedule(salon, weekStart) {
  ...
  console.log("useWeekSchedule input:", { salon, weekStart, monday, sunday });
  ...
  // interval.day mora biti npr. "Pon", "Uto"...
  const dayName = interval.day; 
  const date = new Date(monday); // start from monday
  ...
  console.log("useWeekSchedule output:", { weekSchedule, dayTimes, intervals, label, monday, sunday });
